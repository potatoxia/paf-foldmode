...
# ./launch_foldmode_pipeline.py -l 20 -n 0 -m 80000000000 -p J0218+4232 -c fold_stream.conf -d /beegfs/DENG/docker -u 50000 -g 50000
...
com_line = "docker run --runtime=nvidia -v {:s} -v {:s} -u {:d}:{:d} -e NVIDIA_VISIBLE_DEVICES={:s} -e NVIDIA_DRIVER_CAPABILITIES=all --rm --ulimit memlock={:d} --net=host --name {:s} xinpingdeng/foldmode -c {:s} -n {:d} -l {:f} -d {:s} -p {:s} -v {:s}".format(dvolume, svolume, uid, gid, str(gpu), memsize, dname, cfname, numa, length, ddir, psrname, str(gpu))
#com_line = "docker run -it --runtime=nvidia -v {:s} -v {:s} -u {:s} -e NVIDIA_VISIBLE_DEVICES={:s} -e NVIDIA_DRIVER_CAPABILITIES=all --rm --ulimit memlock={:d} --net=host --name {:s} xinpingdeng/foldmode".format(dvolume, svolume, uid, str(gpu), memsize, dname)
print com_line
